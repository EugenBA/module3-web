...
test_http() {
    ...
    if blog-cli register --username "$TEST_USERNAME" --email "$TEST_EMAIL" --password "$TEST_PASSWORD"; then
    ...
    if blog-cli login --username "$TEST_USERNAME" --password "$TEST_PASSWORD"; then
