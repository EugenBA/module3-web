...
pub(crate) mod grpc_client {
    ...
    use crate::models::models::{Post, Response};
    ...
    impl GrpcClient {
        ...
        async fn create_request<T>(
            ...
            message: T,
            token: Option<String>,
        ) -> Result<Request<T>, BlogClientError> {
        ...
        pub(crate) async fn create_post(
            ...
            if let Some(post) = response.post {
                ...
                Err(BlogClientError::CreatePostError(
                    "No post returned".to_string(),
                ))
            }
        }
        ...
        pub(crate) async fn get_posts(
            ...
            let posts = response.posts.into_iter().map(Post::from).collect();
            Ok(Response {
                id: None,
                posts: Some(posts),
                token: None,
