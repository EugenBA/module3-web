...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let client = BlogClient::new(transport, timeout)
        .await
    ...
    let response = match &cli.command {
        ...
        Commands::Create { title, content } => {
            if let Some(_) = client.get_token().await {
                ...
                Err(BlogClientError::Unauthorized(
                    "Token required. Please login first.".to_string(),
                ))
            }
        }
        Commands::Get { id } => client.get_post(*id).await,
        Commands::Update { id, title, content } => {
            if let Some(_) = client.get_token().await {
                let response = client.get_post(*id).await?;
                if let Some(id) = response.id
                    && let Some(mut post_title) = response.title
                    && let Some(mut post_content) = response.content
                    && id == id
                {
                    ...
                    Err(BlogClientError::InvalidRequest(
                        "Not data post update".to_string(),
                    ))
                }
            } else {
                Err(BlogClientError::Unauthorized(
        ...
        Commands::List { limit, offset } => client.list_posts(Some(*limit), Some(*offset)).await,
    };
    match response {
        Ok(response) => {
